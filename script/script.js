...
function restartGame() {
    const humanScoreList = document.querySelector('.human__scores');
    const computerScoreList = document.querySelector('.computer__scores');
    humanScoreList.innerHTML = "";
    computerScoreList.innerHTML = ""
    humanScore = 0;
    computerScore = 0;
}
...
document.querySelectorAll('.player__move').forEach(press => press.onclick = (e) => {
    game(e.target.name, computerPlay());
    restart();
});
...
function keyGame(e) {
    let playerMove;
    const key = document.querySelector(`.play__key[data-key="${e.keyCode}"]`);
    if (!key) return;
...
const keys = document.querySelectorAll('.play__key');
